...
//
//  Created by Giovanni Saberon on 19/08/18.
...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    let timeLineText = ["READ THE WORD OF GOD", "READ THE WORD OF GOD", "READ THE WORD OF GOD", "READ THE WORD OF GOD"]
    ...
    func getTimelineEndDate(for complication: CLKComplication, withHandler handler: @escaping (Date?) -> Void) {
        let currentDate = NSDate()
        let endDate =
            currentDate.addingTimeInterval(TimeInterval(4 * 60))
    ...
    func getTimelineEntries(for complication: CLKComplication, after date: Date, limit: Int, withHandler handler: @escaping ([CLKComplicationTimelineEntry]?) -> Void) {
        ...
        var nextDate = NSDate(timeIntervalSinceNow: 1 * 60)
    ...
    func getLocalizableSampleTemplate(for complication: CLKComplication, withHandler handler: @escaping (CLKComplicationTemplate?) -> Void) {
        ...
        template.headerTextProvider =
            CLKSimpleTextProvider(text: "Bible")
        template.body1TextProvider =
            CLKSimpleTextProvider(text: "Tap to the read the bible")
